...
> **Note:** this file documents the standalone MCP server. The pristine, unmodified version of this project — exactly as it existed before a multi-agent layer was built on top of it — is preserved at the [`v1-mcp-server`](../../releases/tag/v1-mcp-server) git tag, if you want to see it in complete isolation. See the [root README](../README.md) for how this fits into the larger project.
...
![Claude Desktop conversation example](screenshots/claude-desktop-risk-score.png)
...
![MCP Architecture](screenshots/architecture.png)
...
### Fastest way to see it work: one command

```bash
python scripts/run_option_a_demo.py
```

This installs the package, runs [`demo.py`](../demo.py) — a zero-config CLI script that calls real tools against real, live public APIs, including a deliberate failure case to show graceful error handling — then launches [MCP Inspector](https://github.com/modelcontextprotocol/inspector) for interactive exploration in the browser. Pass `--skip-inspector` to stop after the CLI demo (useful if Node.js isn't installed — the script detects this and prints clear instructions rather than failing confusingly).

`demo.py` is also runnable directly (`python demo.py`) once installed, independent of the wrapper script.
...
![MCP Inspector tool call example](screenshots/inspector-tool-call.png)

![MCP Inspector tool call result example](screenshots/inspector-tool-call-result.png)
...
├── demo.py                     # Zero-config CLI demo (no API keys needed)
...
│   └── license_rules.py
├── scripts/
│   └── run_option_a_demo.py    # One-command install + demo.py + MCP Inspector
├── tests/                      # pytest, fully mocked, no network calls
├── docs/
│   ├── mcp-server.md           # This file
│   └── screenshots/            # Reference screenshots
├── SCORING.md                  # Full risk-scoring methodology
└── README.md                   # Root overview — links here and to the multi-agent layer docs
```